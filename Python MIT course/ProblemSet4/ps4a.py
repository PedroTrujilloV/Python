...
WORDLIST_FILENAME = "F:/Python MIT course/PhytonMIT files/ProblemSet4/words.txt"
...
def getWordScore(word, n):
    ...
    suma = 0
    for let in word:
         suma += SCRABBLE_LETTER_VALUES[let]
    wlen = len(word)
    if wlen >= n:
        return (suma*wlen)+50
    else:
...
def isValidWord(word, hand, wordList):
    ...
    try:
        ...
        for let in word:
            if hand2.get(let, 0)<=0:
                return False
            hand2[let] =  hand2.get(let, 0)-1
    except TypeError:
        return False
    else:
        return word in wordList2
